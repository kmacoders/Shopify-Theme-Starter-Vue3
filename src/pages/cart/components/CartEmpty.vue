<template>
  <div class="cart-empty-page" style="margin-bottom: 100px">
    <h1>{{ titleStr }}</h1>
    <p v-if="isEmpty" class="cart-empty--msg">{{ emptyStr }}</p>

    <a :class="[isEmpty ? 'xo-btn' : '']" href="/">
      <span>{{ continueShoppingStr }}</span>
      <svg
        aria-hidden="true"
        focusable="false"
        role="presentation"
...
const CartEmptyProps = Vue.extend({
  props: {
    isEmpty: Boolean
  }
})
